    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select id="active" name="active" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent @error('active') border-red-500 @enderror">
                    <option value="">Sélectionner un statut</option>
                    <option value="1" {{ old('active', $category->active) == '1' ? 'selected' : '' }}>Actif</option>
                    <option value="0" {{ old('active', $category->active) == '0' ? 'selected' : '' }}>Inactif</option>
                </select>
                @error('active')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
