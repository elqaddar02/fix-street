                ...
                <section class="lg:col-span-3 space-y-4">
                    ...
                    <div class="grid sm:grid-cols-2 gap-5">
                        @forelse ($reports as $report)
                            <article class="rounded-2xl border border-red-200 bg-white p-5 shadow-sm hover:shadow-md transition overflow-hidden">
                                <div class="mb-3 h-40 bg-gradient-to-br from-red-50 to-orange-50 rounded-lg overflow-hidden flex items-center justify-center">
                                    @if($report->image && file_exists(storage_path('app/public/' . $report->image)))
                                        <img src="{{ asset('storage/' . $report->image) }}" alt="{{ $report->title }}" class="w-full h-full object-cover">
                                    @else
